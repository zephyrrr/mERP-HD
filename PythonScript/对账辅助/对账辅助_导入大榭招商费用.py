            ...
            try:
                for row in dt.Rows:
                    ...
                    for i in range(0, 5, 1):
                        fyx = '';je = 0;
                        je = Feng.Utils.ConvertHelper.ToDecimal(row[8 + i]);
                        if (je == None):
                            continue;
                        fyx = fyxs[i];
                        db.ExecuteNonQuery("insert Temp_对账单费用查验(相关人,提单号,箱号,费用项,金额) values ('900005','" + str(row[3]) + "','" \
                                           + str(row[0]) + "','" + str(fyx) + "'," + str(je) + ")");
                        count = count + 1;
                ...
                Feng.ServiceProvider.GetService[Feng.IExceptionProcess]().ProcessWithNotify(System.Exception('提单号' + row[3] + '数据有问题，请查证!', ex));           
